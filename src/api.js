...
app.all('/query', async(req, res) => {
  try {
    ...
    for(let i in targets) {
      let input = JSON.parse(targets[i].target);
      let type = 1;
      let target = {};
...
app.get('/bank/operations', async(req, res) => {
  try {
    const ope = Sort.byDate.sort(await getOperations());
    res.send(ope);
  } catch(e) {
...
app.post('/bank/upload', upload.single('compte'), async (req, res) => {
  try {
    const operations = await bankAccount.read(req.file.path, false);
